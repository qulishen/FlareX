...
class Flare_Image_Loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(self.flare_list)):
            # print(len(self.flare_list[i]),len(self.light_list[i]))
        ...
        if "Flare3D" in flare_path:
            img_merge = torch.cat((flare_img,light_img),dim=0)
            img_merge = self.transform_pair(img_merge)
            flare_img,light_img = torch.split(img_merge,3,dim=0)
            flare =  adjust_gamma(flare_img) -  adjust_gamma(light_img)
            ...
            return {
                ...
                "flare": flare,
                "gamma": gamma,
            }  ## fixme,是否提供组件 flare 作为监督损失
        elif "Flare2D" in flare_path:
    ...
    def load_flare(self, flare_name, flare_path):
        ...
        print("Now we have", len(self.flare_list), "flare images")
